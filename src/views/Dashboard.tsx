...
// ── Root shell — holds nav state, passes to Layout ────────────────────────────
export function Dashboard() {
  const [activeNav, setActiveNav] = useState<NavId>('classifier');
  // Keep a domain ref for the sidebar badge; updated when classifier changes domain
  const [activeDomain, setActiveDomain] = useState<Domain>('spam');
  ...
  function renderPage() {
    switch (activeNav) {
      ...
      default:         return <ClassifierPageWithDomainSync onDomainChange={setActiveDomain} />;
    }
  }

  return (
    <Layout activeDomain={activeDomain} activeNav={activeNav} onNavChange={setActiveNav}>
...
// Thin wrapper so ClassifierPage can report its active domain up to the sidebar badge
function ClassifierPageWithDomainSync({ onDomainChange }: { onDomainChange: (d: Domain) => void }) {
  ...
  const confidence = result ? `${(result.scores[result.predicted_class] * 100).toFixed(1)}%` : '—.—%';
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        {[
          ...
          <StatCard title="TOP CLASS" value={topLabel}
            badge={{ label: result ? 'ACTIVE' : 'IDLE', variant: result ? topBadge : 'neutral' }} arrow />,
          <StatCard title="CONFIDENCE" value={confidence} unit="POST."
            badge={{ label: 'ACTIVE', variant: 'active' }} arrow />,
          <StatCard title="NB VARIANT" value={variant === 'multinomial' ? 'MNB' : 'BNB'}
            subtitle={variant === 'multinomial' ? 'WORD COUNTS' : 'BINARY'}
            badge={{ label: 'ACTIVE', variant: 'active' }} />,
        ].map((card, i) => (
          <motion.div key={i} custom={i} variants={CARD_VARIANTS} initial="hidden" animate="visible" style={{ height: '100%' }}>
            {card}
          </motion.div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
        ...
        <div className="card">
          ...
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-muted)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6 }}>DOMAIN</div>
            <DomainToggle value={domain} onChange={handleDomainChange} />
          </div>
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-muted)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6 }}>NB VARIANT</div>
            <VariantToggle value={variant} onChange={setVariant} />
          </div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-muted)', borderTop: '1px solid var(--border)', paddingTop: 10 }}>
            VOCABULARY MATCH:{' '}
        ...
        <div className="card">
          ...
          {result && result.word_contributions.length > 0 ? (
            result.word_contributions.slice(0, 5).map(wc => (
              <div key={wc.word} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <span style={{
                  fontFamily: 'var(--font-mono)', fontSize: 11, minWidth: 80,
                  color: wc.direction === 'positive' ? 'var(--accent)' : wc.direction === 'negative' ? 'var(--danger)' : 'var(--text-muted)',
                }}>{wc.word}</span>
                <DotBar value={Math.min(1, wc.delta_score / 3)}
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-muted)', minWidth: 40, textAlign: 'right' }}>
                  Δ{wc.delta_score.toFixed(2)}
                </span>
              </div>
            ))
          ) : (
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-muted)', textAlign: 'center', padding: '12px 0' }}>
              — NO SIGNALS YET —
            </div>
          )}
        </div>
